...
- When `CS-10` is in scope, the mutation evidence follows `knowledge/process/stryker-workflow.md`: exact restore/run command, target layers, trace link, recorded score, and below-target follow-up candidates or residual risk are all present.
- Git workflow is respected: no direct `main` commit/push assumptions, PR evidence is present when relevant.
- PR body follows `knowledge/process/git-policy.md`「PR 本文の記載ルール」: written in Japanese, every template section filled, a crystallized 自己レビュー (findings and trade-offs, not just checkboxes), and a fixed-row quantitative summary whose target, actual, and result agree with the command-level evidence.
- Reject opaque evidence: missing actual values labeled `PASS`, unexplained `N/A` / `NOT RUN`, deleted standard rows, or `BELOW TARGET` used outside the non-blocking `CS-10` baseline. For implementation PRs, independently check build/analyzer counts, test counts and each core layer's `CS-07` coverage, architecture/banned API results, format, and conditional mutation evidence.
