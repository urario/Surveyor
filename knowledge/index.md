...
* [DES-0018 Composition Root and DI Detailed Design](design/des-0018-composition-root-and-di.md) - `M13` composition-root detailed design (the single concrete↔abstract seam, `RQ-054`): the DI-mechanism decision (`Microsoft.Extensions.DependencyInjection` with explicit per-assembly registration seams), the headless core-vs-production composition split that makes the injection invariants provable in `UT-0013` without WinUI, the full provider wiring table (provider/selection-key/lifetime/scope) over the `ADR-0002` adapter set, the four mechanically enforced injection invariants (read-only-only adapters, single `IClock`, single `IConfidentialityPolicy`, no real clock in a test config) with fail-fast sanitized wiring diagnostics and paired counter-examples, and the `UT-0013` (#52) / `IMP-0015` (#73) handoff.
