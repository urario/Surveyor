...
| Evidence | Added internal-default `ScreenModel`, `UiElement`, `ElementIdentity`, `ScreenIdentity`, `ScreenStateDiscriminator`, `Availability`, `BoundingRect`, `SupportedPatterns`, `ScreenKey`, and `ElementKey`. Added shared internal `KeyDigest` validation to keep `ScreenKey` and `ElementKey` decoupled. Added public `IdentityMaterial` only as the assembly-boundary fallback token carrier. Added `IFallbackKeyDerivation` as the application-owned port and `Sha256FallbackKeyDerivation` as the minimal M09 policy implementation. Added `DomainValueObjectContractTests` to keep value-object guard/canonical/equality behavior covered by behavior assertions rather than coverage padding. |
| Verification | Targeted green after rebase and quantitative-gate refresh: `dotnet test Surveyor.Unit.slnf --no-restore --collect "XPlat Code Coverage" --results-directory .\TestResults\coverage-pr88-rebased-final` passed 19 tests (`Surveyor.Domain.Tests` 9, `Surveyor.Policy.Tests` 2, `Surveyor.Architecture.Tests` 8). Line coverage: Domain 90.08%, Policy 94.34%, core total 90.61%. Quality gates: `dotnet build Surveyor.Unit.slnf --no-restore -v minimal` passed with 0 warnings/errors and no `CA1501`/`CA1502`/`CA1505`/`CA1506` metric violations; `dotnet format Surveyor.Unit.slnf --verify-no-changes --no-restore` passed; `powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\okf\Validate-Okf.ps1` passed for 39 markdown files. |
