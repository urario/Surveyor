...
> **Version note (2026-07-14, adapter-boundary clarification before `UT-0013` / `IMP-0015`):** [DES-0014](des-0014-discovery-uia-msaa-acquisition-and-read-only-audit.md) now fixes one narrow outward adapter edge: `Surveyor.Adapters.Uia` may reference `Surveyor.Adapters.Discovery` only for `IWindowTargetHandleResolver` and `ResolvedWindowTarget`. Discovery remains the sole writer/owner of the session registry; UIA is the sole raw-handle reader. This supersedes the earlier blanket rule that adapters never reference another adapter. All other cross-adapter edges remain forbidden, and Application/Domain still receive only opaque `TargetReference` values.
...
| Residual Risk | The real MFC/Win32 IT fixture app is built incrementally, so the first scaffold is skeleton-only (`R-OPS-03`); packaging form is `ADR-0002`-blocked, so distribution-related project settings (MSIX/unpackaged, signing for `uiAccess`) are provisional; the exact `net10.0-windows` TFM patch and Windows App SDK version are pinned in `global.json`/`Directory.Build.props` and revisited against the Windows App SDK support matrix. The narrow UIA→Discovery resolver edge must remain mechanically allow-listed and requires Human review if expanded. |
...
| `Surveyor.Adapters.Uia` | Interface Adapters | `M06` | `Surveyor.Adapters.Uia` | `Surveyor.Application`, `Surveyor.Domain`, `Surveyor.Adapters.Discovery` (resolver contract only) |
...
- Adapter/`Reports`/`Policy` → another adapter, `Surveyor.Presentation`, or `Surveyor.App`, except the single `Surveyor.Adapters.Uia` → `Surveyor.Adapters.Discovery` resolver-contract edge fixed by `DES-0014`.
...
2. **`Surveyor.Architecture.Tests`** — an automated test project (xUnit + `NetArchTest.Rules`, or an equivalent reflection-over-loaded-assemblies test) that asserts, in the unattended unit lane:
   ...
   - only `Surveyor.App` references concrete adapter assemblies, except `Surveyor.Adapters.Uia` may reference `Surveyor.Adapters.Discovery` and may consume only the public resolver boundary types named by `DES-0014`;
   - no other adapter-to-adapter reference exists, and the Discovery resolver types are not referenced by Application, Domain, Presentation, Reports, Policy, Capture, or Store.
...
| -- | -- | -- | -- | -- |
| Dependency-direction suite (`Surveyor.Architecture.Tests`) | The inward dependency rule (`RQ-054`) cannot be broken silently | Loaded-assembly reference graph matches the allowed edges; Domain has zero Surveyor references; core has no banned framework namespaces; the sole non-App adapter edge is UIA→Discovery and is limited to the `DES-0014` resolver boundary | Add a `ProjectReference` from `Surveyor.Domain` to `Surveyor.Application`, add Capture→Discovery, or expose the resolver to Application (or add a `Microsoft.UI` using in Domain) → suite fails | Asserting projects merely *compile*; testing only that types exist rather than that forbidden dependencies are absent |
...
- **Narrow adapter coupling:** the UIA→Discovery resolver edge is a deliberate outer-layer exception. Its public surface and consumers must stay mechanically allow-listed; expansion beyond the two `DES-0014` resolver types requires another design revision and Human review.
