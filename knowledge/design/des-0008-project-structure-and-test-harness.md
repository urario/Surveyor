...
> **Version note (2026-07-14, adapter-boundary clarification before `UT-0013` / `IMP-0015`, per DES-0007 §5.3):** [DES-0014](des-0014-discovery-uia-msaa-acquisition-and-read-only-audit.md) now fixes one narrow outward adapter edge: `Surveyor.Adapters.Uia` may reference `Surveyor.Adapters.Discovery`. The only public carrier on that edge is the methodless `DiscoveryUiaBridge`; its raw registry/resolver/result members are `internal` and visible to UIA through the sole production `InternalsVisibleTo` grant. Discovery remains the sole registry writer and UIA the sole raw-handle reader. This supersedes both the earlier blanket cross-adapter ban and the first revision's overly broad public resolver contract. All other cross-adapter edges and production friend grants remain forbidden; Application/Domain still receive only opaque `TargetReference` values.
...
| Residual Risk | The real MFC/Win32 IT fixture app is built incrementally, so the first scaffold is skeleton-only (`R-OPS-03`); packaging form is `ADR-0002`-blocked, so distribution-related project settings (MSIX/unpackaged, signing for `uiAccess`) are provisional; the exact `net10.0-windows` TFM patch and Windows App SDK version are pinned in `global.json`/`Directory.Build.props` and revisited against the Windows App SDK support matrix. The narrow UIA→Discovery project/friend edge must remain mechanically allow-listed and requires Human review if expanded. |
...
| `Surveyor.Adapters.Uia` | Interface Adapters | `M06` | `Surveyor.Adapters.Uia` | `Surveyor.Application`, `Surveyor.Domain`, `Surveyor.Adapters.Discovery` (methodless public bridge + internal resolver through the single friend grant) |
...
- Adapter/`Reports`/`Policy` → another adapter, `Surveyor.Presentation`, or `Surveyor.App`, except the single `Surveyor.Adapters.Uia` → `Surveyor.Adapters.Discovery` bridge edge fixed by `DES-0014`.
...
2. **`Surveyor.Architecture.Tests`** — an automated test project (xUnit + `NetArchTest.Rules`, or an equivalent reflection-over-loaded-assemblies test) that asserts, in the unattended unit lane:
   ...
   - only `Surveyor.App` references the full concrete-adapter set; `Surveyor.Adapters.Uia` may additionally reference `Surveyor.Adapters.Discovery`;
   - Discovery exposes no public raw registry/resolver/result member: `DiscoveryUiaBridge` is public but methodless outside its assembly, while `IWindowTargetHandleRegistry`, `IWindowTargetHandleResolver`, `ResolvedWindowTarget`, and their members remain internal;
   - the only production friend grant is `Surveyor.Adapters.Discovery` → `Surveyor.Adapters.Uia`; no other adapter-to-adapter reference or production `InternalsVisibleTo` exists;
   - a parameterized forbidden-consumer check covers `Surveyor.App`, Application, Domain, Presentation, Reports, Policy, Capture, and Store, and fails if any gains a type/member reference to the Discovery bridge internals. Because `Surveyor.App` already references Discovery at assembly level, this check inspects metadata type/member references rather than only `AssemblyName.GetReferencedAssemblies()`.
...
| `InternalsVisibleTo` | each `src` project → `$(AssemblyName).Tests` + `Surveyor.TestSupport`, granted centrally; one explicit production exception: `Surveyor.Adapters.Discovery` → `Surveyor.Adapters.Uia` | Internal-default accessibility policy (`CS-02` in [Coding Standards](../process/coding-standards.md)): tests reach internals without visibility promotion. The production exception is purpose-limited to the `DES-0014` raw-handle bridge, is asserted as the only production friend grant, and avoids making HWND-bearing contracts public. |
...
| -- | -- | -- | -- | -- |
| Dependency-direction suite (`Surveyor.Architecture.Tests`) | The inward dependency rule and raw-handle consumer boundary (`RQ-054`, `RQ-048`) cannot be broken silently | Loaded-assembly reference graph matches the allowed edges; the sole non-App adapter edge and sole production friend grant are UIA→Discovery; Discovery's raw registry/resolver/result surface is internal; a parameterized metadata-reference assertion covers every forbidden consumer (`App`, Application, Domain, Presentation, Reports, Policy, Capture, Store) | For each forbidden consumer, inject a reference to a bridge-internal type/member → that case fails; additionally make `ResolvedWindowTarget` public, add Capture→Discovery, or add a second production friend grant → suite fails | Checking only assembly-level references (which cannot detect misuse from App because App legitimately references Discovery); sampling only Application/Capture while leaving other consumers untested |
...
- **Narrow adapter coupling:** the UIA→Discovery project/friend edge is a deliberate outer-layer exception. The methodless public bridge, internal raw surface, consumer allow-list, and sole production friend grant must stay mechanically fixed; expanding any of them requires another design revision and Human review.
