...
| `EnableNETAnalyzers` + `AnalysisLevel` | `true`, `latest-All` | All Microsoft CA rules enabled (`CS-05` in [Coding Standards](../process/coding-standards.md)); unsuitable rules are suppressed in `.editorconfig` with a per-rule justification comment |
