...
The source originally predating this closure placed `UiaTargetHandleRegistry`, `uia-target-` minting, and the raw table under UIA. [IMP-0018](../traces/imp-0018-discovery-uia-boundary.md) #113 completed that migration into Discovery while preserving `IMP-0013` as historical evidence. Headless `UT-0013` #52 remains the other prerequisite to `IMP-0015` #73; the migration was not silently absorbed into #73. Registry tokens are session-only, and neither bridge role persists or logs raw handles.
...
- The historical UIA-owned `UiaTargetHandleRegistry` / `uia-target-` boundary is closed by [IMP-0018](../traces/imp-0018-discovery-uia-boundary.md) #113. `IMP-0015` #73 must consume that Discovery-owned boundary and remains separately dependent on headless `UT-0013` #52.
