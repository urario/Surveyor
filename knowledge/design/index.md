...
- [DES-0014 Discovery, UIA/MSAA Acquisition, and Read-Only Audit Detailed Design](des-0014-discovery-uia-msaa-acquisition-and-read-only-audit.md) - Detailed-design package 7 (`M05`/`M06`): read-only discovery status/`TargetRef`/within-session ordering, raw-COM UIA acquisition (`ADR-0002`) with MSAA/`WM_GETTEXT` legacy fallbacks, UIA MTA threading + cooperative cancellation (`R-WIN-02`), the `RD-032` prohibited-pattern→COM-method read-only audit and spy (`RQ-048`/`UT-0005`), the legacy acquisition edge table (`R-WIN-03`), virtualized-tree `NotRealized` handling (`R-GTA-02`), the confidence rubric, rung-1 runtime-id detection (delegated by `DES-0009`), and the minimal-privilege policy (`R-SEC-02`).
