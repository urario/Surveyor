...
- [DES-0015 Capture and Snapshot Correspondence Detailed Design](des-0015-capture-and-snapshot-correspondence.md) - Detailed-design package 8 (`M07`): Windows.Graphics.Capture-primary/PrintWindow-fallback capture (`ADR-0002`); the physical-pixel coordinate contract that closes `DES-0009`'s `BoundingRect` "target-DPI-normalized" delegation, backed by the `TRC-0001`-measured DPI-virtualization finding; the pure, fakeable `BoundingRect`→`RectangleDip` overlay mapping; the `SnapshotRef` population as a derived projection (not a new stored field); the capture failure-mode table (`R-WIN-04`); and multi-monitor/occlusion/offscreen handling.
